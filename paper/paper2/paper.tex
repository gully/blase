...
\begin{abstract}
    ...
    \texttt{blase} is \textbf{run} on 1,314 spectra from \textbf{a rectilinear subset of} the PHOENIX synthetic spectral model grid \textbf{($T_\text{eff}$: [2300, 12000] K, $\log(g)$: [2, 6], [Fe/H]: [-0.5, 0] dex, $\lambda$: [8038, 12849] \AA)}.
    ...
    Gaussian Process minimization is then used to infer stellar parameters by minimizing the root-mean-square loss between input and PHOENIX generator spectra.
    \textbf{In} testing \textbf{210 noise-free synthetic models ($T_{\mathrm{eff}}$: [3000, 11000] K, $\log(g)$: [2, 6], [Fe/H]: [-0.5, 0] dex), we find inference errors of:} $T_{\mathrm{eff}}$\textbf{:} 185 K, $\log(g)$\textbf{:} 0.19, and [Fe/H]\textbf{:} 0.12 dex.
    \textbf{We also publish online} an archive of the \texttt{blase} \textbf{models} of the PHOENIX subset.
...
\begin{figure}
    ...
    \textbf{This plot is much smaller than \autoref{fig:figure3} because} \texttt{blase} detects this line at a select chunk of grid points, leading to \textbf{data only existing between $T_\text{eff}$ 3600 K and 6600 K}.}
    \label{fig:figure4}
\end{figure}
...
\textbf{In \autoref{fig:figure5}, we see a comparison between a PHOENIX generator solar spectrum and two nearby native PHOENIX grid point spectra.}
\begin{figure}
    \centering
    ...
    We can see the spectral line shape be reconstructed mostly between the two native PHOENIX spectra.}
    \label{fig:figure5}
\end{figure}

We can observe in \autoref{fig:figure6} that \textbf{a Fe I line from a} reconstructed solar spectrum is not simply a pixel interpolation between the nearest \textbf{PHOENIX} grid point \textbf{spectra}.
\begin{figure}
    \centering
    \includegraphics[width=\columnwidth]{figure6}
    \caption{\textbf{Comparison between the PHOENIX generator's line interpolation and a standard pixel intrpolation of the 10123.66 \AA \ He II spectral line.}}
    \label{fig:figure6}
\end{figure}
...
The speedup factor in parallel mode compared to KORG is then the same speedup factor from the serial mode.}
...
\begin{table}[h!]
    ...
    \begin{tabular}{ll}
        ...
        \hline
    \end{tabular}
    \caption{\textbf{Specifications of the} machine used for computation.
...
\section{Conclusion}
In this study, we have presented a proof-of-concept algorithm that \textbf{deals with} a subset of the PHOENIX spectral model grid \textbf{([Fe/H]: [-0.5, 0] dex, $T_\text{eff}$: [2300, 12000] K, $\log(g)$: [2, 6], $\lambda$: [8038, 12849] \AA)}.
...
We introduce and test our \textbf{GP minimization} algorithm \textbf{to infer stellar parameters.} 
\textbf{Testing on 210 noise-free synthetic models ($T_{\mathrm{eff}}$: [3000, 11000] K, $\log(g)$: [2, 6], [Fe/H]: [-0.5, 0] dex) yields} average absolute deviations from true values of 185 K in $T_\mathrm{eff}$, 0.19 in $\log(g)$, and 0.12 dex in [Fe/H].
