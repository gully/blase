...
\begin{abstract}
    ...
    \authorcomment1{Say how many spectra you analyze in the ``PHOENIX subset''.  State that an outcome of this work is the precomputed training.}
...
We chose the widely-adopted PHOENIX synthetic spectral model grid \authorcomment{cite?} as the basis for this study. The framework can be straightforwardly extended to any other model grid in the future, including substellar atmosphere grids such as Sonora \citep{bobcat, cholla, diamondback, elfowl}, for now we limit our scope to a subset of the PHOENIX grid.

\authorcomment1{whoops, I repeated this above.  Key idea is to put some numbers here.}
...
\authorcomment1{state the number of spectra in the subset if you have not already done so.  optionally state how many teff, logg, fe/h spacings.}
...
three-step preprocessing pipeline, identical to \authorcomment1{blase paper 1}.
\begin{enumerate}
...
grid point \citep{pytorch}. The total disk space these files took up is 465 MB.\authorcomment1{Restate the number of grid points for good measure, }.
...
\authorcomment1{I do not understand this.  Maybe consider removing?}
...
reconstruct spectra from an array of input coordinates \authorcomment1{which input coordinates-- fundamental properties or line properties?}. Therefore, there
