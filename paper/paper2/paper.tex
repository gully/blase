...
\begin{abstract}
    ...
    Here we introduce a hybrid ML (HML) method combin\textbf{ing} automatic differentiation, interpolation, and Bayesian optimization to infer stellar parameters \textbf{of} stellar spectra.
...
\begin{figure*}
    ...
    \caption{\textbf{Residual plots for the GP minimization inference algorithm, all plotted with respect to $T_\text{eff}$.
...
\textbf{In the low-temperature regime, our median absolute $T_\text{eff}$ residual is 60 K, about three times that of MINESweeper \citep{minesweeper} and \texttt{astroNN} \citep{leung2019}, being within an order of magnitude, but about 60 times larger than ASPCAP \citep{ASPCAP}.
