...
\begin{abstract}
    ...
    \texttt{blase} is \textbf{run} on 1,314 spectra from \textbf{a rectilinear subset of} the PHOENIX synthetic spectral model grid \textbf{([Fe/H]: [-0.5, 0] dex, $T_\text{eff}$: [2300, 12000] K, $\log(g)$: [2, 6], $\lambda$: [8038, 12849] \AA)}.
...
\begin{figure*}
    ...
    \textbf{Since} \texttt{blase} did not detect a spectral line here, \textbf{so} we artificially populate those points with lines that have $\ln(a) = -1000$ \textbf{during interpolation}.
...
We introduce and test our \textbf{GP minimization} algorithm \textbf{to infer stellar parameters}, with average absolute deviations from true values of 185 K in $T_\mathrm{eff}$, 0.19 in $\log(g)$, and 0.12 dex in [Fe/H].
