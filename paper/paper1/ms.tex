...
But overall, the telluric model cloning performs adequately, with \censorbox{X percentage} error-per-pixel overall, and only \censorbox{zz} error-per-pixel when restricted to the atmospheric windows.  We therefore urge some caution when applying \texttt{blas\'e} to telluric modeling in the most saturated regions.  \authorcomment1{An obvious and good solution is to intialize the model in log flux.  We may revise the scheme to do so.}
...
\begin{eqnarray}
    \mathsf{S}_{\rm ext}(\bm{\lambda}_S) = \mathsf{S}_{\rm clone}(\lambda_\mathrm{c} - \frac{RV}{c}\lambda_\mathrm{c}) * \zeta \left(\frac{\bm{v}}{v\sin{i}}\right) \label{eqn_convolution}
\end{eqnarray}
...
Operationally, the Doppler shift gets applied to the line center positions and not the individual wavelength grid point coordinates, $\bm{\lambda}_S$. This choice yields a key benefit: it makes the $RV$ autodiff-aware, meaning that an infinitesimal change to the RV value can easily be sensed through backpropagation by affecting only the line center positions.  The strategy allows relatively low computational and memory cost since it comports with the sparse matrix implementation.

\subsection{Joint Stellar and Telluric Model}
The telluric model and stellar models possess distinct pixel sampling and coordinates.  The design of \texttt{blas\'e} permits the telluric model to be re\"evaluated at any wavelength coordinate vector, provided that it adequately samples the underlying lines.  We choose to evaluate the telluric model at the native resolution of the stellar model.  We then simply multiply the stellar model by the tunable telluric transmission:

\begin{eqnarray}
    \mathsf{M}_{\rm joint} = \mathsf{S}_{\rm ext}(\bm{\lambda}_S) \odot \mathsf{T}_{\rm clone}(\bm{\lambda}_S)
\end{eqnarray}
...
\begin{eqnarray}
    \mathsf{M}(\bm{\lambda}_D) = \resample{\Big[\mathsf{M}_{\rm joint}(\bm{\lambda}_S) * g(R) \Big]} \label{eqn_final_model}
\end{eqnarray}
