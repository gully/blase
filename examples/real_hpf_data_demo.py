...
def train_emulator(emulator, target):
    """Train the emulator"""
    ...
    optimizer = optim.Adam(
        filter(lambda p: p.requires_grad, emulator.parameters()), 0.01, amsgrad=True
    )
    n_epochs = 200
    losses = []
...
torch.save(model.state_dict(), "model_T4100g3p5_prom0p01_HPF.pt")
