...
emulator = SparsePhoenixEmulator(
    wl_native, flux_native, prominence=prominence, wing_cut_pixels=1000, device=device
)
emulator.to(device)

# print(emulator.indices_2D.shape)
# print(emulator.indices_2D)
# print(emulator.indices_1D.shape)
# print(emulator.wl_native.to("cpu"))[emulator.indices_2D.to("cpu")]

wl_native = emulator.wl_native.clone().detach().to(device)
wl_active = wl_native.to("cpu")[emulator.active_mask.to("cpu").numpy()]
target = (
    emulator.flux_native.clone().detach().to("cpu")[emulator.active_mask.cpu().numpy()]
)
...
for epoch in t_iter:
    emulator.train()
    output = emulator.forward()
    # high_res_model = output[emulator.active_mask]
    high_res_model = torch.masked_select(output, emulator.active_mask)
